...
public class ProjectMeetingMinuteService {
	...
	@Transactional
	public void updateMinute(ProjectMeetingMinuteDto dto, List<Long> participants) {
	    // 기존 회의록 조회
	    ProjectMeetingMinute entity = projectMeetingMinuteRepository.findById(dto.getMeetingMinuteNo())
	        .orElse(null);

	    // GPT 요약
	    String summary = gptService.summarizeMeetingContent(dto.getMeetingContent());

	    // 엔티티 업데이트
	    entity.update(dto.getMeetingTitle(), dto.getMeetingContent(), summary);

	    // 기존 참여자 매핑 삭제
	    List<ProjectMeetingMinuteMapping> existing = projectMeetingMinuteMappingRepository.findByMeetingMinuteNo(dto.getMeetingMinuteNo());
	    projectMeetingMinuteMappingRepository.deleteAll(existing);

	    // 새 참여자 매핑 저장
	    List<ProjectMeetingMinuteMapping> newMappings = new ArrayList<>();
	    for (Long empNo : participants) {
	        newMappings.add(ProjectMeetingMinuteMapping.builder()
	            .meetingMinuteNo(dto.getMeetingMinuteNo())
	            .employeeNo(empNo)
	            .build());
	    }
	...
	public void deleteMinute(Long minuteNo) {
		// 원본 엔티티 조회
		ProjectMeetingMinute entity = projectMeetingMinuteRepository.findById(minuteNo).orElse(null);
